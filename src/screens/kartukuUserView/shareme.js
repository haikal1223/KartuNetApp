...
const kartukuUserShareMe = ({navigation}) => {
  ...
  const listMenu = [
    {
      image: PhoneBookIcon,
      title: 'Phone book',
      onPress: () => Linking.openURL(`tel://${userKartuku.phone}`),
    },
    {
      image: QrCodeIcon,
      title: 'QR Code',
      onPress: () => navigation.navigate('qrCodeUserViewDetail'),
    },
    {
  ...
  const shareToSocialMedia = async () => {
    try {
      const url = `${BASE_URL}/${userKartuku.slug}`;
      const title = `Kartu Digital ${userKartuku.name}`;
      const message = `Kartu Digital ${userKartuku.name}.\n\nUntuk Lihat Kartu Digital user ${userKartuku.name} klik link ini ${url}`;
...
kartukuUserShareMe.propTypes = {
  navigation: PropTypes.shape({
    navigate: PropTypes.func.isRequired,
  }).isRequired,
};

export default kartukuUserShareMe;
