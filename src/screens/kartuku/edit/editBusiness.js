...
const editBusinessScreen = ({route, navigation}) => {
  ...
  const fetchBusinessData = async () => {
    try {
      ...
      setBusinessPosisi(posisi);
      setBusinessDeskripsi(deskripsi);
      setBusinessLogo({
        fileName: `${BASE_URL}/public/storage/business/${logo}`,
        fileUri: `${BASE_URL}/public/storage/business/${logo}`,
        fileType: `image/${logo.split('.')[1]}`,
      });
      ...
      const {data: kecamatanList} = await axios.get(
        `${BASE_URL}/api/get_kecamatan/${kota_id}`,
      );

      const {data: kelurahanList} = await axios.get(
        `${BASE_URL}/api/get_kelurahan/${kecamatan_id}`,
      );

      const listProvinsi = formatSelectBox(listProvince.data);
      const listKota = provinsi_id
        ? formatSelectBox(listCity.data)
        : [
            {
              label: 'Kosong',
              value: null,
      ...
      const listKecamatan = kota_id
        ? formatSelectBox(kecamatanList.data)
        : [
            {
              label: 'Kosong',
              value: null,
      ...
      const listKelurahan = kecamatan_id
        ? formatSelectBox(kelurahanList.data)
        : [
            {
              label: 'Kosong',
              value: null,
      ...
      setBusinessAlamat(alamat);
      setBusinessProvinsi(provinsi_id ? Number(provinsi_id) : null);
      setBusinessKota(kota_id ? Number(kota_id) : null);
      setBusinessKecamatan(kecamatan_id ? Number(kecamatan_id) : null);
      setBusinessKelurahan(kelurahan_id ? Number(kelurahan_id) : null);
      setBusinessKodePos(kode_pos);
      ...
      // form 4
      setBusinessFacebook(facebook);
      setBusinessTwitter(twitter);
      setBusinessLinkedin(linkedin);
      setBusinessInstagram(instagram);
      setBusinessYoutube(youtube);

      // form 5
      setBusinessTokopedia(tokopedia);
      setBusinessShopee(shopee);
      setBusinessBukalapak(bukalapak);
  ...
  const onSubmitBusinessData = async () => {
    try {
      ...
      formData.append('bisnis_id', businessId);
      ...
      await axios.post(
        `${BASE_URL}/api/business/post_update_step_one/${user.id}`,
        formData,
        options,
      );
